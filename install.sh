...
usage() { echo "Usage: sudo -v ; curl -k https://rclone.org/install.sh | sudo bash [-s beta]" 1>&2; exit 1; }
...
if [ -z "$install_beta" ]; then
    current_version=$(curl -k -fsS https://downloads.rclone.org/version.txt)
else
    current_version=$(curl -k -fsS https://beta.rclone.org/version.txt)
fi
